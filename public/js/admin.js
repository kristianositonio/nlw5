...
function call(id) {
  ...
  socket.emit("admin_list_messages_by_user", params, (messages) => {
    ...
    const divMessages = document.getElementById(`allMessages${conn.user_id}`);
    ...
    messages.forEach((message) => {
      ...
      if (message.admin_id === null) {
        ...
        createDiv.innerHTML = `<span>${conn.user.email}</span>`;
...
socket.on("admin_receive_message", (data) => {
  const conn = connectionsUser.find(connection => connection.socket_id = data.socket_id);
  const divMessages = document.getElementById(`allMessages${conn.user_id}`);
